...
function click() {
    ...
    let themeicon = document.getElementsByClassName('theme-button-img-span')
    if (buttonImg.value) {
        buttonimg.style.transform = "translate(19px)"
        document.body.setAttribute('data-theme', "dark");
        themeicon[1].classList.remove("opacity-0");
        themeicon[0].classList.add("opacity-0");
        localStorage.setItem('theme', false);
        buttonImg.value = false
    } else {
        buttonimg.style.transform = "translate(0px)"
        document.body.setAttribute('data-theme', "light");
        themeicon[0].classList.remove("opacity-0");
        themeicon[1].classList.add("opacity-0");
...
<template>
    <button @click.prevent="click" class="theme-button">
        <div id="button-img">
            <span class="theme-button-img">
                <span class="theme-button-img-span">
                ...
                <span class="theme-button-img-span opacity-0">
...
.theme-button {
    width: 40px;
    height: 22px;
    background-color: var(--theme-button-bgcolor);
    border-radius: 20px;
    position: relative;
}
#button-img {
    width: 18px;
    height: 18px;
    background-color: var(--theme-button-span-bgcolor);
    border-radius: 50%;
